...
numtrials = 10; %number of times we add noise and calculate the dmd 

r0 = 40e-9; % radius of final sphere
r1 = 20e-9; % initial semiminor axis (value used when varying tau)
...
tau1vals = linspace(1,1,1); % values over which to vary tau1 
tau2vals = linspace(1,1,1); % values iver which to vary tau2
...
for tau1_iter = tau1vals 
    ...
    for tau2_iter = tau2vals
        ...
        for ii = 1:numtrials
            ...
            try 
                [w,e,b] = multioptdmd3(X,t,r,0.05); % w: dmd modes, e:eigenvalues, b:coefficients
        ...
        tempmat(abs(tempmat) > max(tau1_iter,tau2_iter)*10) = 0; % remove obviously erroneous results
        for jj = 1:2
            row = tempmat(jj,:);
