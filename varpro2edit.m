...
% sometimes the absolute value elements get so large that it exceeds 
% the limit of floating point numbers, if this happens set them as the 
% largest/smallest floating point number possible
phimat(real(phimat) == Inf) = realmax;
phimat(real(phimat) == -Inf) = realmin;
