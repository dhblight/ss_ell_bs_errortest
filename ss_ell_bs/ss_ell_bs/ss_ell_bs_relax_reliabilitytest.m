%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Script to test the reliability of ss_ell_bs_relax.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
tau1vals = linspace(1,91,10); % relaxation time for small spheres -> ellipsoid
tau2vals = linspace(100,1000,10); % relaxation time for ellipsoid -> big sphere
...
for tau1_iter = tau1vals
    ...
    for tau2_iter = tau2vals
        ...
        % generate clean data
        Xclean = ss_ell_bs_intensity(c0,nsph,r0,r1,ra,rb,qgrid,T,Mss,M,tau1_iter,tau2_iter);
        ...
        for r = rankvals
            ...
            for kk = 1:numtrials
                ...
                [w,e,b,~,evals] = multioptdmd(X,t,r,0.032);
